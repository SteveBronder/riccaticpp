...
def Bremer237(l, n, eps, epsh, outdir, algo):
    ...
    if algo is Algo.PYRICCATICPP:
        ...
        with open(outputf, 'w') as f:
            ...
            for line in lines:
                f.write(line)
            f.write("{}, {}, {}, {}, {}, {}, {}".format("riccaticpp",\
                    l, eps, round_to_n(3, yerr), round_to_n(3, runtime),\
                    (yerr < errref), "(n = {}; p = {}; epsh = {})".format(n, p, epsh)))
            f.write("\n")
        ...
        for i in range(N):
            xs, ys, dys, ss, ps, stypes, _, _ = riccati.solve(info, xi, xf, yi, dyi, eps = eps, epsh = epsh, hard_stop = True)
        end = time.time_ns()
        ys = np.array(ys)
        # Compute statistics
        runtime = (end - start)*1e-9/N

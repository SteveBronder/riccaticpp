...
def Bremer237(l, n, eps, epsh, outdir, algo):
    ...
    if algo is Algo.PYRICCATICPP:
        ...
        for i in range(N):
            xs, ys, dys, ss, ps, stypes, _, _ = riccati.solve(
        ...
        kummeroutput = subprocess.run(
            ["./{}".format(kummerscript), str(l), str(eps)], capture_output=True
        )
        ...
        os.system(
            'matlab -batch "global aeval; {}({}, {}, {}, {}); exit" '.format(
                matlabscript, l, eps, N, outputf
            )
        )
        os.chdir("../../")
...
def joss_fig(outdir):
    ...
    sol = solve_ivp(
        f, [-1, 1], [0, l], method=method, rtol=eps, atol=atol, t_eval=t_eval
    )
    ...
    outputfs = [
        outdir + "bremer237-{0}.txt".format(method)
        for method in ["rdc", "kummer", "oscode", "wkbmarching"]
    ]
    dfs = []

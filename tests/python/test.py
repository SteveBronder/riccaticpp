...
def test_solve_airy():
    ...
    yi = complex(sp.airy(-xi)[0] + 1j*sp.airy(-xi)[2])
    dyi = complex(-sp.airy(-xi)[1] - 1j*sp.airy(-xi)[3])
    xs, ys, dys, ss, ps, stypes, yeval, dyeval = ric.evolve(info, xi, xf, yi, dyi, eps = eps, epsilon_h = epsh, init_stepsize = 0.01)
    xs = np.array(xs)
    ys = np.array(ys)
...
def test_solve_airy_backwards():
    ...
    dyi = complex(-sp.airy(-xi)[1] - 1j*sp.airy(-xi)[3])
    xs, ys, dys, ss, ps, stypes, yeval, dyeval = ric.evolve(info, xi, xf, yi, dyi,\
                                                eps = eps, epsilon_h = epsh, init_stepsize = -0.01, \
                                                hard_stop = True)
    xs = np.array(xs)
    ys = np.array(ys)
    ytrue = np.array([mpmath.airyai(-x) + 1j*mpmath.airybi(-x) for x in xs])
...
def test_denseoutput_backwards_xfor():
    ...
    xeval = np.linspace(xi, xf, Neval)
    xs, ys, dys, ss, ps, stypes, yeval, dyeval = ric.evolve(info, xi, xf, yi, dyi,\
                                                       eps = eps, epsilon_h = epsh,\
                                                       x_eval = xeval, init_stepsize = -0.01, \
                                                       hard_stop = True)
    ytrue = np.array([mpmath.airyai(-x) + 1j*mpmath.airybi(-x) for x in xeval])
    yerr = np.abs((ytrue - yeval)/ytrue)
    maxerr = max(yerr)
...
def test_solve_burst():
    ...
    epsh = 1e-12
    info = ric.Init(w, g, 16, 32, 32, 32)
    xs, ys, dys, ss, ps, types, yeval, dyeval = ric.evolve(info, xi, xf, yi, dyi, eps = eps, epsilon_h = epsh)
    xs = np.array(xs)
    ys = np.array(ys)
    ytrue = bursty(xs)
...
def test_osc_evolve():
    ...
    while xi < xf:
        status, x_next, h_next, osc_ret, yeval, dyeval, \
          dense_start, dense_size = ric.osc_evolve(info, xi, xf, yi, dyi, \
                                                   eps = eps, epsilon_h = epsh, init_stepsize = hi)
        if status is False:
            break
        xs.append(x_next)
        ys.append(osc_ret[1])
...
def test_nonosc_evolve():
    ...
    # Not necessary here because info.x is already xi, but in general it might be:
    while xi < xf:
        status, x_next, h_next, nonosc_ret, yeval, dyeval, \
                           dense_start, dense_size = ric.nonosc_evolve(info, xi, xf, yi, dyi, eps = eps, epsilon_h = epsh, init_stepsize = hi)
        if status is False:
            break
        xs.append(x_next)

...
TEST_F(Riccati, evolve_schrodinger_nondense_fwd_full_optimize) {
  ...
  Eigen::Matrix<double, 0, 0> x_eval;
    ...
    const double right_boundary = -left_boundary;
    constexpr double midpoint = 0.5;
    auto init_step = riccati::choose_nonosc_stepsize(info, left_boundary, midpoint - left_boundary, epsh);
    auto left_res
        = riccati::evolve(info, left_boundary, midpoint, yi, dyi, eps, epsh, init_step, x_eval, true, LogLevel::WARNING);
    auto left_y_est = Eigen::Map<Eigen::Matrix<std::complex<double>, -1, 1>>(
        std::get<1>(left_res).data(), std::get<1>(left_res).size());
  ...
  constexpr std::array bounds{std::make_pair(416.5, 417.5), std::make_pair(1035.0, 1037.0),
    std::make_pair(21930.0, 21939.0), std::make_pair(471100.0, 471110.0)};
  constexpr std::array reference_energy{417.056, 1035.544, 21932.783, 471103.777};
  int sentinal = 0;
  for (auto& bound : bounds) {
    auto solve_ans = boost::math::tools::brent_find_minima(
      energy_difference, bound.first, bound.second, std::numeric_limits<double>::digits / 2);
    auto abs_target_diff = std::abs(solve_ans.first - reference_energy[sentinal]);
    sentinal++;
